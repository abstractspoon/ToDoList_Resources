...
Updates:
150128a : TG First version provided to Dan for review
...
- - Use Selected tasks.
- - The output columns must include the task ID, title, comments, and estimate hours.

This is for Estimates only. Another sheet should be used to provide clients with
actual time versus estimates, status, etc.
...
= Company producing the estimate.
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="pVersion" select="'150128a'" />
  ...
  <xsl:template match="TODOLIST">
    <xsl:element name="html">
      <xsl:variable name="hdr">
        <xsl:choose>
          <xsl:when test="string-length(@PROJECTNAME) &gt; 0">
...
using tg_Estimates XSL v]]></xsl:text><xsl:value-of select="$pVersion" />
		<xsl:text disable-output-escaping="yes">
<![CDATA[-->]]>
		</xsl:text>
        <xsl:choose>
      ...
      <xsl:element name="body">
        ...
        <xsl:element name="div">
          ...
          <xsl:element name="h2">
            <xsl:value-of select="concat($pCompanyName1,' Project Estimate for ',$hdr)" />
          </xsl:element>
        </xsl:element>
...
<ul>
<li>This is an estimate, not a quote. Actual time per task may vary.</li>
...
<li>Parent tasks are broken down into sub-tasks. Parent tasks show the sum of all child tasks.</li>
  ...
  <xsl:template name="buildRow">
    ...
    <xsl:element name="tr">
      ...
      <!-- Back batch of TD cells has Estimates, maybe more -->
      <xsl:call-template name="get_Hours" >
        <xsl:with-param name="level" select="$level" />
      </xsl:call-template>

      <!-- Now follow the data with empty cells to balance out original indentation -->
      <xsl:call-template name="add_table_cells" />
  ...
  <!-- Now add cells for the numeric data -->
  <xsl:template name="get_Hours">
    <xsl:param name="level" />
    <!-- Estimate -->
  ...
  <!-- This sets the class of an existing tag. It's assumed that the tag is created just before this is called. -->
